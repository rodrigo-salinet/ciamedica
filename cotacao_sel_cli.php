...
if ($id_cliente != '') {
	...
	$sel_nivel = trim(@$_GET['sel_nivel']);;
	$sel_tipo = trim(@$_GET['sel_tipo']);;
	...
	if (@$_SESSION['id_cotacao'] != '') {
		$sel_id_cli = "";
		if (@mysql_num_rows($cliente) > 0) {
			$sql_string = "UPDATE COTACOES SET $campos_valores WHERE ID=". @$_SESSION['id_cotacao'];
			$cli = 'Atualizado';
		}
	} else {
	...
	if ($cli == 'Inserido') {
		$sql_cotacao_temp = "SELECT * FROM COTACOES ORDER BY ID DESC";
		$cotacao_temp = mysql_query($sql_cotacao_temp,$conexao);
		if (@mysql_num_rows($cotacao_temp) > 0) {
			$_SESSION['id_cotacao'] = @mysql_result($cotacao_temp,0,'ID');
		}
		@mysql_free_result($cotacao_temp);
	}
}
$cotacao = '';
if (trim(@$_SESSION['id_cotacao']) != '') {
	$cotacao = '&id_cotacao=' . $_SESSION['id_cotacao'];
}
header("Location: cotacao_editar.php?id_tipo=$id_tipo&msg=" . urlencode($msg) . $cotacao);
?>