...
function valida_form() {
	var frm_cad_cob_cli = document.getElementById('frm_cad_cob_cli');
...
$sql_cob_cliente = "SELECT * FROM COBRANCA_CLIENTE WHERE ID_CLIENTE=$id_cliente";
...
if (@mysql_num_rows($cob_cliente) > 0) {
	$contato = @mysql_result($cob_cliente,0,'CONTATO');
	$endereco = @mysql_result($cob_cliente,0,'ENDERECO');
	$numero = @mysql_result($cob_cliente,0,'NUMERO');
	$complemento = @mysql_result($cob_cliente,0,'COMPLEMENTO');
	$bairro = @mysql_result($cob_cliente,0,'BAIRRO');
	$municipio = @mysql_result($cob_cliente,0,'MUNICIPIO');
	$uf = @mysql_result($cob_cliente,0,'UF');
	$cep = @mysql_result($cob_cliente,0,'CEP');
	$ddd_fone = @mysql_result($cob_cliente,0,'DDD_FONE');
	$fone = @mysql_result($cob_cliente,0,'FONE');
	$ddd_fax = @mysql_result($cob_cliente,0,'DDD_FAX');
	$fax = @mysql_result($cob_cliente,0,'FAX');
	$ddd_celular = @mysql_result($cob_cliente,0,'DDD_CELULAR');
	$celular = @mysql_result($cob_cliente,0,'CELULAR');
	$email = @mysql_result($cob_cliente,0,'EMAIL');
	$obs = @mysql_result($cob_cliente,0,'OBS');
} else {
...
?>
<DIV CLASS="nome_pag">Editar Dados de Cobrança</DIV>
<FORM ACTION="cliente_cob_atualizar.php" ID="frm_cad_cob_cli" METHOD="POST" NAME="frm_cad_cob_cli"> 
