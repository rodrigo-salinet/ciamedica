...
<?
$hdn_id_fornecedor = trim(@$_POST['hdn_id_fornecedor']);
...
$sql_string = "INSERT INTO FORNECEDORES ($campos) VALUES ($valores)";

$sql_fornecedor = "SELECT * FROM FORNECEDORES WHERE ID=$hdn_id_fornecedor";
$fornecedor = mysql_query($sql_fornecedor,$conexao);
$existe = false;
if (@mysql_num_rows($fornecedor) > 0) {
	$existe = true;
	$sql_string = "UPDATE FORNECEDORES SET $campos_valores WHERE ID=$hdn_id_fornecedor";
}
...
if (mysql_query($sql_string,$conexao)) {
	...
	$msg = "Fornecedor $foi com sucesso.";
} else {
	$executar = 'cadastrar';
	if ($existe == true) {
		$executar = 'atualizar';
	}
	$msg = "Ocorreu um problema ao $executar fornecedor.";
}
...
$redir = '';
if ($hdn_id_fornecedor != '') {
	$redir = "&id_fornecedor=$hdn_id_fornecedor";
}

header("Location: fornecedor_editar.php?msg=" . urlencode($msg) . $redir);
?>
