...
if ($id_cotacao == '') {
	$id_cotacao = trim(@$_POST['hdn_id_cotacao']);
}
	...
	for  ($n = 0; $n < @mysql_num_rows($estoque); $n++) {
		...
		if ($txt_quantidade != '') {
			...
			if ($id_tipo != '') {
				...
				$campos_valores .= "ID_TIPO=$id_tipo";
			} else {
				if ($campos_valores != '') {
					$campos_valores .= ',';
				}
			...
			if ($id_cotacao != '') {
				...
				$campos_valores .= "ID_TABELA=$id_cotacao";
			} else {
				if ($campos_valores != '') {
					$campos_valores .= ',';
				}
...
header("Location: cotacao_editar.php?id_tipo=$id_tipo&msg=" . urlencode($msg) . '&id_cotacao=' . $id_cotacao);
?>