...
if ($id_venda == '') {
	header("Location: venda_todas.php?msg=" . urlencode('Identificador da venda não encontrado. Selecione a venda primeiro.'));
}
$sql_venda = "SELECT * FROM VENDAS WHERE ID=$id_venda";
$venda = mysql_query($sql_venda,$conexao);
?>
<DIV CLASS="nome_pag">Selecionar Ítens da Venda p/ Cliente <?=@mysql_result($venda,0,'CLI_NOME');?></DIV>
<FORM ACTION="venda_adic_itens.php?id_venda=<?=$id_venda;?>" ID="frm_itens" METHOD="POST" NAME="frm_itens">
...
if (@mysql_num_rows($produtos) > 0) {
	...
	for ($n = $linha_inicial; $n < $linha_final; $n++) {
		$sql_item_venda = "SELECT * FROM ITENS WHERE ID_TIPO=2 AND ID_TABELA=$id_venda AND ID_PRODUTO=" . mysql_result($produtos,$n,'ID');
		$item_venda = mysql_query($sql_item_venda,$conexao);
		if (@mysql_num_rows($item_venda) == 0) {
?>
	<TR>
