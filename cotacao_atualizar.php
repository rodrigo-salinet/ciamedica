...
if ($sel_nivel != '') {
	...
	$campos_valores .= "ID_NIVEL=$sel_nivel";
} else {
	if ($campos_valores != '') {
		$campos_valores .= ',';
	}
	$campos_valores .= "ID_NIVEL=NULL";
}
...
if ($rdg_aceita != '') {
	...
	$campos_valores .= "ACEITA='$rdg_aceita'";
} else {
	if ($campos_valores != '') {
		$campos_valores .= ',';
	}
	$campos_valores .= "ACEITA=NULL";
}
...
$sql_string = "INSERT INTO COTACOES ($campos) VALUES ($valores)";

$sql_cotacao = "SELECT * FROM COTACOES WHERE ID=$hdn_id_cotacao";
$cotacao = mysql_query($sql_cotacao,$conexao);
$existe = false;
if (@mysql_num_rows($cotacao) > 0) {
	$existe = true;
	$sql_string = "UPDATE COTACOES SET $campos_valores WHERE ID=$hdn_id_cotacao";
}
...
if (mysql_query($sql_string,$conexao)) {
	...
	if ($existe == true) {
		...
		$sql_itens = "SELECT * FROM ITENS WHERE ID_TIPO=$id_tipo AND ID_TABELA=$hdn_id_cotacao";
		...
		if (@mysql_num_rows($itens) > 0) {
			...
			for  ($n = 0; $n < @mysql_num_rows($itens); $n++) {
				...
				if ($txt_quantidade_item != '') {
					...
					$sql_atualizar = "UPDATE ITENS SET $campos_valores WHERE ID_TIPO=$tipo AND ID_PRODUTO=$id_produto AND ID_TABELA=$hdn_id_cotacao";
		...
		$sql_cotacao = "SELECT * FROM COTACOES ORDER BY ID DESC";
		$cotacao = mysql_query($sql_cotacao,$conexao);
		$id_cotacao = @mysql_result($cotacao,0,'ID');
		@mysql_free_result($cotacao);
	}
	$msg = "Cotação $foi com sucesso.";
	if ($tem_item == true) {
		if ($erro == false) {
		...
		$msg .= " Não há item cadastrado para esta cotação.";
	}
} else {
	$executar = 'cadastrar';
	if ($existe == true) {
	...
	$msg = "Ocorreu um erro ao $executar cotação.";
}
...
$redir = '';
if ($hdn_id_cotacao != '') {
	$redir = "&id_cotacao=$hdn_id_cotacao";
}
if (@$id_cotacao != '') {
	$redir = "&id_cotacao=$id_cotacao";
}
...
header("Location: $pag.php?id_tipo=$id_tipo&msg=" . urlencode($msg) . $redir);
?>
