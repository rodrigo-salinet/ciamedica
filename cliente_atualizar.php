...
if ($txt_cfop != '') {
	...
	$campos_valores .= "CFOP='$txt_cfop'";
} else {
	if ($campos_valores != '') {
		$campos_valores .= ',';
	}
	$campos_valores .= "CFOP=NULL";
}
...
$sql_string = "INSERT INTO CLIENTES ($campos) VALUES ($valores)";
...
if (@mysql_num_rows($cliente) > 0) {
	$existe = true;
	$sql_string = "UPDATE CLIENTES SET $campos_valores WHERE ID=$hdn_id_cliente";
}
...
if (mysql_query($sql_string,$conexao)) {
	...
	$msg = "Cliente $foi com sucesso.";
} else {
	$executar = 'cadastrar';
	if ($existe == true) {
		$executar = 'atualizar';
	}
	$msg = "Ocorreu um problema ao $executar cliente.";
}
...
header("Location: cliente_editar.php?msg=" . urlencode($msg) . $redir);
?>
